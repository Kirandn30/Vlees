...
const SingleOrder: React.FC<Props> = ({ route }) => {
    ...
    useEffect(() => {
        ...
        if (data) {
            ...
            //@ts-ignore
            Firebase.firestore().collection("Orders").doc(route.params.orderId).get().then((res) => setOrder(res.data()))
        }
    }, [Orders])
    ...
    return (
        <View className='min-h-screen bg-white '>
            ...
            <View className='space-y-3 bg-white p-3'>
                <Text className='font-medium text-lg text-center'>Order Details</Text>
                {order.items.map((item: any) => (
                    <View key={item.selectedVariant.id} className='flex flex-row justify-between rounded-lg'>
